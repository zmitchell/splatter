...
impl Egui {
    ...
    pub fn with_epi_frame<F>(&mut self, proxy: splatter::app::Proxy, f: F)
    where
    ...
    pub fn do_frame_with_epi_frame<F>(&mut self, proxy: splatter::app::Proxy, f: F) -> egui::Output
    where
        F: FnOnce(&CtxRef, &mut epi::Frame),
    {
        self.begin_frame_inner();
