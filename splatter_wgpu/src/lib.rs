...
//!
//! **WebGPU** is the portable graphics specification that splatter targets allowing us to write code
...
//! adding some additional items that makes `wgpu` easier to use alongside splatter.
//!
...
//!
//! Note that when using `splatter_wgpu` via `splatter::wgpu`, both features are enabled by default.
//!
//! Useful links:
//!
...
/// splatter's default WGPU backend preferences.
...
pub fn shader_from_spirv_bytes(
    ...
    let desc = ShaderModuleDescriptor {
        label: Some("splatter_shader_module"),
        source,
    };
