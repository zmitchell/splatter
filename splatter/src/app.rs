...
pub struct App {
    ...
    ///
    /// NOTE: `down` this is tracked by the splatter `App` so issues might occur if e.g. a key is
    ...
    M: 'static,
{
    ...
    /// lifetime of your splatter program from start to exit.
    ///
    ...
    /// however splatter also provides a default `Event` type that should cover most use cases. This
    ...
    E: LoopEvent,
{
    ...
    /// application loop modes available in splatter and how they behave.
    ///
...
impl App {
    ...
    /// devices.
    ///
    /// By maintaining a map of active adapters and their established devices, splatter allows for
    ...
    /// `DeviceDescriptor`s, splatter will automatically share devices between windows where
    ...
    /// The name of the splatter executable that is currently running.
...
//
// This is undoubtedly the hairiest part of splatter's code base. This is largely due to the fact
// that it is the part of splatter where we marry application and user input events, loop timing,
    ...
    E: LoopEvent,
{
    ...
    } = *winit_event
    {
        ...
        // If the event can be represented by a simplified splatter event, check for relevant user
        // functions to be called.
        if let Some(simple) =
            event::WindowEvent::from_winit_window_event(event, win_w, win_h, scale_factor)
        {
            // splatter window events.
