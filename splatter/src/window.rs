...
/// Surface configuration for which splatter will provide a default if unspecified.
///
...
impl SurfaceConfigurationBuilder {
    ...
    ///
    /// By default, splatter will attempt to select the ideal present mode depending on the current
    /// app `LoopMode`.
...
impl<'app> Builder<'app> {
    ...
    ///
    /// If `msaa_samples` is unspecified, the first default value that splatter will attempt to use
    ...
    /// render to a multisampled intermediary image allowing splatter to take care of resolving the
    ...
    ///
    /// In splatter, if multiple functions require being called for a single kind of event, the more
    ...
    /// than splatter's simplified `event::WindowEvent`s.
    ///
    /// ## Event Function Call Order
    ///
    ...
    pub async fn build_async(self) -> Result<Id, BuildError> {
        ...
        // If the title was not set, default to the "splatter - <exe_name>".
        if !title_was_set {
            if let Ok(exe_path) = env::current_exe() {
                if let Some(os_str) = exe_path.file_stem() {
                    if let Some(exe_name) = os_str.to_str() {
                        let title = format!("splatter - {}", exe_name);
        ...
        ))]
        {
            ...
            window = window.with_class("splatter".to_string(), "splatter".to_string());
        }
...
impl Window {
    ...
    /// versa.
    ///
    /// Throughout splatter, you will see "logical pixels" referred to as "points", and "physical
